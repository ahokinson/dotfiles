...
}:
{
  imports = [
    ...
    (selfPath "modules/nixos/networking/mdns.nix")
  ...
  # No modules/nixos/core/boot.nix: that's systemd-boot/EFI, which a Pi 4 doesn't
  # have - raspberry-pi-4.base owns boot.loader.raspberry-pi instead.
  # No modules/nixos/user/home-manager.nix: single-purpose appliances, never
  ...
  # minimal boxes by design. modules/nixos/networking/mdns.nix is the one exception,
  ...
  # is already the real access gate (key-only, per modules/nixos/networking/ssh.nix).
  ...
  # presentDevicePolicy "allow" in modules/nixos/security/usbguard.nix assumes
  ...
  # rescaled again as hosts/nixos/asahi/common.nix does for its ten-core pair - a
