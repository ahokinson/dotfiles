...
let
  dockApps = import (selfPath "home/common/theme/dock-apps.nix");
  inherit (import (selfPath "home/common/theme/icons.nix") { inherit inputs pkgs selfPath; }) mkIcon;
  ...
  logo =
    if isApple then
      ...
      } (selfPath "home/common/assets/asahi-apple.svg")
    else
      recolor {
      ...
      let
        tile = mkIcon (
          ...
          // lib.optionalAttrs (app.vendoredIcon or false) {
            glyphDir = selfPath "home/common/assets";
          }
        );
      in
      # One generated tile, filed under every Icon= key a desktop entry for
      # this app could use (home/common/theme/dock-apps.nix's extraLinuxIconNames)
