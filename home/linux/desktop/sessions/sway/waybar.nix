...
let
  ...
  # Same recolor home/linux/desktop/icons/default.nix does for COSMIC's app-button
  ...
  logo = pkgs.writeText "waybar-logo.svg" (
    if isApple then
      lib.replaceStrings
        ...
        (builtins.readFile (selfPath "home/common/assets/asahi-apple.svg"))
    else
      lib.replaceStrings
        [
          "#699ad7"
  ...
  # home/linux/desktop/icons/default.nix installs this exact package and has both
...
in
{
  # Global accent is mauve (home/common/theme/catppuccin.nix); overridden to match
  # COSMIC's own blue accent override (home/linux/desktop/sessions/cosmic/theme.nix), same as
  ...
  programs.waybar = {
    ...
    settings.mainBar = {
      ...
      # cosmic-panel-config's shared panel fields (home/linux/desktop/sessions/cosmic/panel.nix's
      ...
      "image#logo" = {
        ...
        # presentation, home/linux/desktop/sessions/cosmic/panel.nix's panel-button applet
        # config).
        size = 24;
      };
    ...
    style = lib.mkAfter ''
      /* text::body's fixed px constant (libcosmic/src/widget/text.rs).
         ...
         (32) <= PanelSize::S's (40) - independent of home/common/theme/fonts.nix's
