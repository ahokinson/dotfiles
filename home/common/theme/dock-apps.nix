...
# modules/darwin/system, home/linux/desktop/sessions/cosmic/panel.nix and home/common/theme/icons.nix.
#
...
# icon either way.
# vendoredIcon marks simpleIcon as a filename under home/common/assets
# instead of a glyph in the simple-icons input (see slack; the glyph itself
# lives in home/linux/desktop/icons/default.nix's mkIcon call).
...
# skips pinning any app that omits it, for one with no macOS build here.
{
  ...
  # the box.
  brlcad = {
    linuxDesktopId = "archer.desktop";
    linuxIconName = "brlcad";
    simpleIcon = "brlcad";
    vendoredIcon = true;
    hue = "pink";
  };
  ...
  slack = {
    ...
    linuxIconName = "slack";
    # Asahi hosts run slacky instead (home/linux/packages/default.nix); its .desktop
