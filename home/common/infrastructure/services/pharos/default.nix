...
let
  ...
  baseConfig = import (selfPath "home/common/infrastructure/services/pharos/config-base.nix");
  palette = import (selfPath "home/common/theme/palette.nix");
in
{
  home.packages = [ pkgs.pharos ];

  # A worked plugin example, vendored from pharos's examples/; the package
  # ships only the binary. Its guard ids (risk/policy/judgement) must keep
  # matching cerberus's head names, which key the violations file it reads.
  # If they drift, every count silently renders zero.
  xdg.configFile."pharos/guards.ts".text = ''
