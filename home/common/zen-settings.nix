...
let
  ...
  # macOS traffic lights for Zen's window controls, Linux only - on macOS Zen
  # already gets the real ones from the system.
  #
  # This has to be done in chrome CSS rather than through the GTK theme that
  # styles ghostty's controls (home/linux/cosmic/gtk.nix): Zen draws these
  # buttons itself, and only consults GTK's -moz-gtk-csd-* values to decide
  # whether to show them and in which order. That's why the button *placement*
  # from gtk-decoration-layout lands while the appearance doesn't.
  #
  # Class names verified against the shipped browser/omni.ja rather than
  # recalled: titlebar-buttonbox-container, titlebar-button, and
  # titlebar-{close,min,max,restore}.
  zenTrafficLights = ''
