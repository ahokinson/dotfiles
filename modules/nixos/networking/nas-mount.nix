...
# hosts/nixos/raspberry-pi/pi-nas/default.nix's smbpasswd draw the same line by hand instead.
...
{ config, username, ... }:
{
  fileSystems."/mnt/developer" = {
    ...
    options = [
      "username=${username}"
      # 1000, not a dynamic lookup: modules/nixos/core/user.nix never pins a UID
